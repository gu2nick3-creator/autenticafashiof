    ...
    >
      ...
      <div className="relative aspect-[3/4] bg-cream overflow-hidden mb-4">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-1.5">
          {product.isNew && (
            <span className="bg-primary text-primary-foreground text-[10px] tracking-luxury uppercase font-body px-2.5 py-1">
              Novo
            </span>
          )}
          {product.onSale && (
            <span className="bg-gold text-white text-[10px] tracking-luxury uppercase font-body px-2.5 py-1">
              Promoção
            </span>
          )}
          {product.bestSeller && !product.isNew && (
            <span className="bg-rose text-white text-[10px] tracking-luxury uppercase font-body px-2.5 py-1">
              + Vendido
            </span>
          )}
        </div>
      ...
      <div>
        ...
        <Link to={`/produto/${product.slug}`}>
          <h3 className="font-display text-base md:text-lg font-medium leading-tight hover:text-gold transition-colors">
            {product.name}
          </h3>
        </Link>
        {currentCustomer ? (
          <>
            ...
            <p className="text-[11px] text-muted-foreground font-body mt-1">
              ou 6x de R$ {(displayPrice / 6).toFixed(2).replace('.', ',')} sem juros
            </p>
          </>
        ) : (
          <div className="mt-3 rounded-2xl border border-border bg-muted/30 p-3">
            <p className="text-sm font-medium font-body">Faça login ou registre-se para visualizar o preço.</p>
            <div className="mt-3 flex flex-wrap gap-2">
              <Link to="/login" className="inline-flex min-h-10 items-center justify-center rounded-full border border-foreground px-4 text-[11px] font-medium uppercase tracking-luxury transition-colors hover:bg-foreground hover:text-background">Entrar</Link>
              <Link to="/cadastro" className="inline-flex min-h-10 items-center justify-center rounded-full border border-border px-4 text-[11px] font-medium uppercase tracking-luxury transition-colors hover:border-foreground">Cadastrar</Link>
            </div>
          </div>
        )}
          ...
          {currentCustomer ? (
            <>
              <p className="mt-1 text-sm font-semibold font-body">
                10 pares: R$ {resaleBundlePrice.toFixed(2).replace('.', ',')}
              </p>
              <p className="text-[11px] text-muted-foreground font-body mt-1">Preço liberado para revenda após login.</p>
            </>
          ) : (
            <p className="mt-2 text-[11px] text-muted-foreground font-body">Entre na sua conta para visualizar também o valor de revenda.</p>
          )}
        </div>
      </div>
    </motion.div>
